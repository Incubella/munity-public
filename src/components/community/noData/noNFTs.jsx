...
import { GET_MARKETPALCE_URL } from "@/utils";
export default function NoNFTs({
  userCommunityBalance,
  handleOpenBuyForm,
  areNFTs,
}) {
  const { getCommunityDetailsById, address, chainId } = useWeb3Context();
  ...
  return (
    ...
    >
      ...
      <CustomCard
        ...
        price={
          data &&
          `${ethers.formatEther(data?.price)} ${
            chainId != "" ? availableChains[chainId].symbol : "--"
          } `
        }
        ...
        url={GET_MARKETPALCE_URL(chainId, userCommunityBalance?.id)}
      />
    </Box>
  );
}
