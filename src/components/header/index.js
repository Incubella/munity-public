...
export default function Header() {
  ...
  useEffect(() => {
    if (chainId !== "") {
      ...
      setSelectedChain(
        `${IS_TEST_MODE ? ChainIds.Ethereum : ChainIds.Polygon}`
      ); //defai
    }
  }, [chainId, connected]);
  useEffect(() => {
    const checkIfWalletBanned = async (walletAddress) => {
      try {
        const response = await fetch(`/api/banned-wallets`);
        const { bannedWallets } = await response.json();
        console.log(bannedWallets, walletAddress);
        return bannedWallets.includes(walletAddress);
      } catch (error) {
        console.error("Error checking banned wallets:", error);
        return false;
      }
    };
    ...
    const handleUserConnection = async () => {
      if (address) {
        const isBanned = await checkIfWalletBanned(address);
        if (isBanned) {
          disconnectHandle();
          router.push("/banned");
        } else {
          console.log("it is not banned");
          getCommunityByUserAddress();
        }
      }
    };
  ...
  const mobileMenuItems = [
    { name: "Explore", url: "https://app.munity.ai" },
    !hasCommunity && {
  ...
  const goToLanding = () => router.push("https://munity.ai");
