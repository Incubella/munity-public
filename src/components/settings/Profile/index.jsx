...
export const getUserOwnedAccessKey = async (
  userAddress,
  chainId,
  setAllNftData
) => {
  try {
    ...
    if (response?.raw?.result.length > 0) {
      ...
      const promises = response.raw.result.map(async (item) => {
        ...
        return {
          avatarImage: uriData?.avatarImage,
          img: uriData?.image,
          title: uriData?.name,
          amount: item.amount,
      ...
      console.log(
        "getUserOwnedAccessKey dataWithParsedMetadata",
        dataWithParsedMetadata
      );
      setAllNftData && setAllNftData(dataWithParsedMetadata);
      return dataWithParsedMetadata;
    } else {
      setAllNftData && setAllNftData([]);
    }
  } catch (e) {
    console.error(e);
    setAllNftData && setAllNftData([]);
...
export default function Profile({ userAddress, setLoading, userData }) {
  ...
  useEffect(() => {
    if (address != "" && chainId != "") {
      getUserOwnedAccessKey(address, chainId, setAllNftData);
    }
  }, [address, chainId]);
