...
const SellItemForm = ({
  ...
  const { getCommunityNftPayAmount, buyCommunityNft, address,chainId } =
    useWeb3Context();
  ...
  useEffect(() => {
    setChainSymbol(`${chainId!=""? availableChains[chainId].symbol:"--"}`)
  }, [chainId])
  ...
  useEffect(() => {
    // Update the ETH/MATIC to USD conversion whenever the price in ETH/MATIC changes or the quantity changes
      ...
      <Box sx={{ mt: 2, mb: 2 }}>
        ...
        {type === "sell" && !buyDataLoading && (
          <>
            ...
            <SingleFee
              feeName={
                type === "sell" ? `Total Earnings (${chainSymbol})` : `Total Cost (${chainSymbol})`
              }
              fee={totalEarnings}
              icon={svgIconETH}
              cost={ethInUsd}
