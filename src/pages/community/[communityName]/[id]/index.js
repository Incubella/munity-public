...
export const fetchCommunityCreatorData = async (creatorId, setCreatorData) => {
  try {
    const response = await fetch(`/api/users/${creatorId}`);
    const data = await response.json();
    setCreatorData && setCreatorData(data.data);
    return data.data;
  } catch (error) {
...
const SingleCommunityPage = () => {
  ...
  const { communityname, id } = router.query; // Retrieve the community ID from the URL
  ...
  useEffect(() => {
    ...
    const fetchCommunityData = async (currentId) => {
      try {
        ...
        await fetchCommunityCreatorData(dataBackend.user_id, setCreatorData);
